//#region src/shadow-utils/dom.js
...
function getClosestElement(start, selector) {
	...
	if (start instanceof ShadowRoot) return getClosestElement(start.host, selector);
	const assignedSlot = start.assignedSlot;
	return assignedSlot ? start.matches(selector) ? start : getClosestElement(assignedSlot, selector) : start.closest(selector) ?? (start.getRootNode() instanceof ShadowRoot ? getClosestElement(start.getRootNode().host, selector) : null);
}
...
function getParentElement(node) {
	if (!node) return null;
	if (typeof node.assignedElements !== "function" && node.assignedSlot) return node.assignedSlot;
	const root = node.getRootNode();
	if (root instanceof ShadowRoot) return node.parentElement ?? root.host;
...
function getLastElementDescendant(container) {
	let descendant = null;
	for (let lastChild = getLastElementChild(container); lastChild; lastChild = getLastElementChild(lastChild)) descendant = lastChild;
	return descendant;
}
//#endregion
export { getClosestElement, getLastElementDescendant, getParentElement, nodeContains };
