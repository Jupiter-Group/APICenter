import { nodeContains } from "./dom.mjs";
//#region src/shadow-utils/mutation-observer.js
...
var ShadowMutationObserver = class ShadowMutationObserver {
	...
	static #overrideAttachShadow(win) {
		...
		Element.prototype.attachShadow = function(options) {
			...
			for (const shadowObserver of ShadowMutationObserver.#shadowObservers) shadowObserver.#addSubObserver(shadowRoot);
			return shadowRoot;
		};
	...
	#callbackWrapper = (mutations, observer) => {
		for (const mutation of mutations) if (mutation.type === "childList") {
			...
			for (let i = 0; i < removed.length; i++) this.#walkShadows(removed[i], true);
			for (let i = 0; i < added.length; i++) this.#walkShadows(added[i]);
		}
		this.#callback(mutations, observer);
	};
	#addSubObserver(shadowRoot) {
		if (!this.#options || !this.#callback || this.#subObservers.has(shadowRoot)) return;
		if (this.#options.subtree && nodeContains(this.#root, shadowRoot)) {
			...
			if (this.#isObserving) subObserver.observe(shadowRoot, this.#options);
	...
	#removeSubObserver(shadowRoot) {
		...
		if (!this.#subObservers.size) this.#subObservers.clear();
	}
	disconnect() {
		this.#isObserving = false;
		this.#options = {};
		ShadowMutationObserver.#shadowObservers.delete(this);
		for (const shadowRoot of this.#subObservers.keys()) this.#removeSubObserver(shadowRoot);
	...
	#walkShadows(target, remove) {
		...
		if (target === doc) target = doc.body;
			...
			if (shadowRoot) {
				...
				} else this.#addSubObserver(shadowRoot);
				return;
			}
		}
		doc.createTreeWalker(target, NodeFilter.SHOW_ELEMENT, { acceptNode: (node) => {
			if (node.nodeType === Node.ELEMENT_NODE) if (remove) {
				...
				if (shadowRoot) this.#addSubObserver(shadowRoot);
			}
	...
	takeRecords() {
		...
		for (const subObserver of this.#subObservers.values()) records.push(...subObserver.takeRecords());
		return records;
	}
};
