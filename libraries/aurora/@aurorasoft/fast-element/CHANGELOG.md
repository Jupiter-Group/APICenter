# Change Log - @aurorasoft/fast-element
...
- Remove the fast-kernel multi-kernel modes. FAST no longer supports configuring isolated or version-scoped kernels via the script attribute. (7559015+janechu@users.noreply.github.com)
...
- Remove FASTElement.compose from the public API. (7559015+janechu@users.noreply.github.com)
...
- Move optional helpers to dedicated flat fast-element subpath exports such as @aurorasoft/fast-element/children.js, @aurorasoft/fast-element/repeat.js, @aurorasoft/fast-element/two-way.js, @aurorasoft/fast-element/signal.js, @aurorasoft/fast-element/attribute-map.js, and @aurorasoft/fast-element/observer-map.js while keeping controller and definition internals on the root entrypoint. (7559015+janechu@users.noreply.github.com)
- Remove lifecycle callback APIs and add registry and hydration promise APIs. (7559015+janechu@users.noreply.github.com)
- Change default attribute-name-strategy from none to camelCase. (7559015+janechu@users.noreply.github.com)
- Remove deprecated declarative event e support. (7559015+janechu@users.noreply.github.com)
- Make declarative runtime setup lazy and change @aurorasoft/fast-element/debug.js to require an explicit enableDebug() call. (7559015+janechu@users.noreply.github.com)
- Remove hydration view template side effect and unused export paths. (7559015+janechu@users.noreply.github.com)
- Move declarative HTML APIs into @aurorasoft/fast-element/declarative.js, expose schema map helpers from extension subpaths, and remove the @aurorasoft/fast-html package. (7559015+janechu@users.noreply.github.com)
- Simplify hydration markers to data-free sequential format (fe:b, fe:/b, fe:r, fe:/r, fe:e, fe:/e), replace regex parsing with string equality checks, and use a single data-fe attribute instead of the three old formats. SSR output format changed. (7559015+janechu@users.noreply.github.com)
...
- Normalize and clean up package export paths: relocate subpath targets to their feature modules, alphabetize the exports map, drop the styles/templating/node-observation subpaths, narrow css/html to their template tags, add a dedicated notifier subpath, and remove the test resolution condition. (863023+radium-v@users.noreply.github.com)
...
- Improve hydration mismatch behaviour: render() falls back to client rendering when SSR view boundaries are empty, repeat() reconciles SSR/client item-count mismatches by creating missing client views or removing extra SSR ranges, and unrecoverable mismatches throw via a pluggable HydrationDiagnostic. Opt in to the rich 'Expected / Received' format with the SSR HTML snippet and structured expected/received fields on HydrationBindingError / HydrationTargetElementError by passing enableHydration({ debugger: hydrationDebugger() }) (hydrationDebugger is exported from '@aurorasoft/fast-element/hydration.js'). (7559015+janechu@users.noreply.github.com)
- Auto-escape `{` and `}` inside `<code>` elements so binding-like syntax in code samples renders as literal text (mirrors webui-press and the server-side `escape_code_sample_elements` pass in `@aurorasoft/fast-build`, which additionally handles `<f-when>`/`<f-repeat>` directive tags). (7559015+janechu@users.noreply.github.com)
...
- Propagate shadowroot attributes from f-template to declarative shadow DOM template. (7559015+janechu@users.noreply.github.com)
- Modularize hydration and expose lifecycle callbacks via enableHydration() and declarativeTemplate(). (7559015+janechu@users.noreply.github.com)
...
- Expose declarative HTML syntax constants from declarative utilities. (7559015+janechu@users.noreply.github.com)

### Patches

- Allow duplicate f-template names to keep the first template assignment. (7559015+janechu@users.noreply.github.com)
- Avoid mutating render template bindings. (7559015+janechu@users.noreply.github.com)
- Restore CaptureType generic type capture for template directives. (7559015+janechu@users.noreply.github.com)
- Preserve view context when rebinding the same source. (7559015+janechu@users.noreply.github.com)
- Align @attr({ mode: 'boolean' }) and booleanConverter with native HTML boolean attribute semantics. (7559015+janechu@users.noreply.github.com)
- Harden DOM policy guard merging and unsafe URL filtering. (7559015+janechu@users.noreply.github.com)
...
- chore: add interfaces to sideEffects (mmansour@aurorasoft.com)
...
- remove shadowOptions observation (machi@aurorasoft.com)
...
- Update @aurorasoft/fast-element to major version (13071055+chrisdholt@users.noreply.github.com)
...
- BREAKING: update fast-element export paths to include extensions (chhol@aurorasoft.com)
- update fast-element exports to be explicit (chhol@aurorasoft.com)
...
- fix: update compiler to ensure first and last child references are defined (chhol@aurorasoft.com)
...
- update return type for dangerousHTML to DangerousHTMLDirective (chhol@aurorasoft.com)
- chore: remove deprecated APIs from fast-element (rob@bluespire.com)
- chore: cleanup fast-element interfaces (roeisenb@aurorasoft.com)
- refactor(fast-element): replace dangerousHTML with html.partial (roeisenb@aurorasoft.com)
- feat: add ability to inline templates (roeisenb@aurorasoft.com)
...
- feat: add kernel mode with isolation by default (roeisenb@aurorasoft.com)
...
- chore: move ViewBehaviorOrchestrator to utilities (roeisenb@aurorasoft.com)
- feat: add new DOM Policy protection throughout (roeisenb@aurorasoft.com)
...
- fix: prevent circular reference errors when stringifying DOM nodes controlled by FAST's rendering engine (roeisenb@aurorasoft.com)
...
- fix: tokenList bindings track adds and removes from multiple sources (roeisenb@aurorasoft.com)
- refactor: remove duplication and re-organize binding behavior logic (roeisenb@aurorasoft.com)
...
- fix: prevent event callbacks when controller is not bound (roeisenb@aurorasoft.com)
...
- fix: repeat update view context positioning (roeisenb@aurorasoft.com)
...
- fix: observation kernel missing binding observer disconnect method (roeisenb@aurorasoft.com)
- feat: less surprising class bindings w/ more flexibility for developers (roeisenb@aurorasoft.com)
- fix: don't allow registering FASTElement itself as a web component (roeisenb@aurorasoft.com)
...
- fix: ensure composed templates receive the existing execution context (roeisenb@aurorasoft.com)
...
- fix: correct view state when processing unbindables during rebind (roeisenb@aurorasoft.com)
- fix: do not crash when null/undefined is provided literally in templates (roeisenb@aurorasoft.com)
...
- feat: improve parent typing and fix repeat context type bug (roeisenb@aurorasoft.com)
- feat: prepare fast-element for scoped element registries (roeisenb@aurorasoft.com)
- persist totalAvailableViews to compare against removeIndex (wendywendy@aurorasoft.com)
- make ElementsFilter type arguments optional (863023+radium-v@users.noreply.github.com)
- fix(fast-element): attributes properly located in inheritance hierarchy (roeisenb@aurorasoft.com)
- Adds PendingTask community protocol (nicholasrice@users.noreply.github.com)
- feat: new behavior and context API design (roeisenb@aurorasoft.com)
- feat: implement getAsync code path for dependency injection (roeisenb@aurorasoft.com)
...
- feat: add new state, ownedState, and computedState APIs (roeisenb@aurorasoft.com)
...
- refactor: remove dependency of DI on FASTElement (roeisenb@aurorasoft.com)
...
- feat: extract all error messages from context and DI (roeisenb@aurorasoft.com)
- feat: introduce new binding API (roeisenb@aurorasoft.com)
- Enabled DOM Container fallback functions when a container cannot be located. (roeisenb@aurorasoft.com)
- feat: add render directive, renderWith decorator, and RenderInstruction (roeisenb@aurorasoft.com)
...
- feat: enable render, repeat, and when template helpers to handle static data in addition to the already supported dynamic bindings (roeisenb@aurorasoft.com)
- fix: FASTElementDefinition#isDefined should be true only after define (roeisenb@aurorasoft.com)
- feat: move testing and dependency injection to fast-element (roeisenb@aurorasoft.com)
- fix: warn when host bindings are accidentally applied on non CE views (roeisenb@aurorasoft.com)
- feat: perf and safety improvements to fast element definition (roeisenb@aurorasoft.com)
...
- doc: add note to FASTElement.metadata API (roeisenb@aurorasoft.com)
- when recycle is set to false while itemBinding stays the same, views should be recreated (wendy.hsu@aurorasoft.com)
- feat: simplify execution context to align closer with v1 (roeisenb@aurorasoft.com)
- feat: ergo improvements to context, array length, and metadata (roeisenb@aurorasoft.com)
- feat: move optional bindings out of rollup and list as exports (roeisenb@aurorasoft.com)
- fix: make SyntheticViewTemplate type a string so it is generally usable (roeisenb@aurorasoft.com)
- feat: implement W3C WC community context protocol and integrate with DI (roeisenb@aurorasoft.com)
...
- `class` - Bindings to `class` are now more nuanced. Binding directly to `class` will simply set the `className` property. If you need to bind to `class` knowing that manual JS will also manipulate the `classList` in addition to the binding, then you should now bind to `:classList` instead. This allows for performance optimizations in the simple, most common case. (roeisenb@aurorasoft.com)
- `Behavior` and `ViewBehavior` - `Behavior` now requires an `ExecutionContext` for `unbind`. Behaviors can be used for elements or views. `ViewBehavior` has been introduced for use exclusively with views, and provides some optimization opportunities. (roeisenb@aurorasoft.com)
- `RefBehavior` has been replaced with `RefDirective`. The directive also implements `ViewBehavior` allowing a single directive instance to be shared across all template instances that use the ref. (roeisenb@aurorasoft.com)
- Removed `SlottedBehavior` and `ChildrenBehavior` have been replaced with `SlottedDirective` and `ChildrenDirective`. These directives allow a single directive instance to be shared across all template instances that use the ref. (roeisenb@aurorasoft.com)
- Removed `AttachedBehaviorHTMLDirective` and `AttachedBehaviorType` since they are no longer used in the new directive/behavior architecture for ref, slotted, and children. (roeisenb@aurorasoft.com)
- Renamed `Notifier#source` to `Notifier#subject` to align with other observable terminology and prevent name clashes with `BindingObserver` properties. (roeisenb@aurorasoft.com)
- feat: create unified API for parsing markers (roeisenb@aurorasoft.com)
- allow null values in attributeChangedCallback (nicholasrice@users.noreply.github.com)
- feat: add splice strategies for array observation (roeisenb@aurorasoft.com)
- feat: handle existing shadow roots when upgrading (roeisenb@aurorasoft.com)
- fix: enable safer type inference on the repeat helper (roeisenb@aurorasoft.com)
- Set prerelease version (nicholasrice@users.noreply.github.com)
- refactor: separate update queue from DOM and fix architectural layers (roeisenb@aurorasoft.com)
- feat: aspected html directive exposes metadata (roeisenb@aurorasoft.com)
- `HTMLDirective` - The `targetIndex: number` property has been replaced by a `targetId: string` property. The `createBehavior` method no longer takes a target `Node` but instead takes a `BehaviorTargets` instance. The actual target can be looked up on the `BehaviorTargets` instance by indexing with the `targetId` property. (roeisenb@aurorasoft.com)
- `compileTemplate()` - Internals have been significantly changed. The implementation no longer uses a TreeWalker. The return type has change to an `HTMLTemplateCompilationResult` with different properties. (roeisenb@aurorasoft.com)
- refactor: move template/style resolution to lazy getter (roeisenb@aurorasoft.com)
- refactor: enable pluggable template compiler for SSR scenarios (roeisenb@aurorasoft.com)
- feat: enable pluggable style handling strategies (roeisenb@aurorasoft.com)
- fix(fast-element): do not notify splices for changes pre-subscription (roeisenb@aurorasoft.com)
- feat: new directive registration/identification model (roeisenb@aurorasoft.com)
- fix: prevent duplicative array observation patch (roeisenb@aurorasoft.com)
- Adds Aspect as public export (nicholasrice@users.noreply.github.com)
- feat: expose official Markup and Parser APIs (roeisenb@aurorasoft.com)
- refactor: refine binding mode types (roeisenb@aurorasoft.com)
- feat: add two-way binding (roeisenb@aurorasoft.com)
- fix: defend against for/in use on arrays (roeisenb@aurorasoft.com)
- fix: merge bug related to moved APIs (roeisenb@aurorasoft.com)
- feat: enable multiple instances of fast-element on a page at once (roeisenb@aurorasoft.com)
...
- feat: enable array length observation (roeisenb@aurorasoft.com)
- feat: add utilities and hooks, and change exports (roeisenb@aurorasoft.com)
- refactor: new design for execution context (roeisenb@aurorasoft.com)
- chore: fast-element package and build modernization (roeisenb@aurorasoft.com)
- feat: new CSSDirective design (roeisenb@aurorasoft.com)
- chore: configure fast-element for internal stripping (roeisenb@aurorasoft.com)
- feat: enable synchronous dom updates for SSR (roeisenb@aurorasoft.com)
- refactor: extract polyfill and polyfill-like code to an optional module (roeisenb@aurorasoft.com)
- feat: add warn/error message infrastructure (roeisenb@aurorasoft.com)
- `View` and `HTMLView` - Type parameters added to enable strongly typed views based on their data source. The constructor of `HTMLView` has a new signature based on changes to the compiler's output. Internals have been cleaned up and no longer rely on the Range type. (roeisenb@aurorasoft.com)
- `ElementViewTemplate`, `SyntheticViewTemplate`, and `ViewTemplate` - Added type parameters throughout. Logic to instantiate and apply behaviors moved out of the template and into the view where it can be lazily executed. Removed the ability of the `render` method to take a string id of the node to render to. You must provide a node. (roeisenb@aurorasoft.com)
...
- update to typescript 4.6.2 and update ARIAMixin typings (chhol@aurorasoft.com)
...
- feat: enable multiple fast-element instances in browser at once (roeisenb@aurorasoft.com)
...
- add recyle option to repeat directive (scomea@aurorasoft.com)
...
- update fast eslint package version (chhol@aurorasoft.com)
...
- feat(fast-element): expose a method for processing the update queue (roeisenb@aurorasoft.com)
...
- perf(Controller): reduce object allocation needed for isConnected prop (roeisenb@aurorasoft.com)
...
- feat(fast-element): make the task queue resilient to task exceptions (roeisenb@aurorasoft.com)
...
- fix: ensure that custom events are cancelable by default (roeisenb@aurorasoft.com)
- perf: improve startup time and reduce library size (roeisenb@aurorasoft.com)

## [1.0.2](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@1.0.1...@aurorasoft/fast-element@1.0.2) (2021-04-06)
...
## [1.0.1](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@1.0.0...@aurorasoft/fast-element@1.0.1) (2021-03-16)
...
# [1.0.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.22.2...@aurorasoft/fast-element@1.0.0) (2021-03-06)
...
## [0.22.2](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.22.0...@aurorasoft/fast-element@0.22.2) (2021-02-08)

**Note:** Version bump only for package @aurorasoft/fast-element





## [0.22.1](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.22.0...@aurorasoft/fast-element@0.22.1) (2021-02-08)
...
# [0.22.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.21.1...@aurorasoft/fast-element@0.22.0) (2021-01-30)
...
## [0.21.1](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.21.0...@aurorasoft/fast-element@0.21.1) (2020-12-16)
...
# [0.21.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.20.0...@aurorasoft/fast-element@0.21.0) (2020-12-02)
...
# [0.20.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.19.1...@aurorasoft/fast-element@0.20.0) (2020-11-19)
...
## [0.19.1](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.19.0...@aurorasoft/fast-element@0.19.1) (2020-10-14)
...
# [0.19.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.18.0...@aurorasoft/fast-element@0.19.0) (2020-09-28)
...
# [0.18.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.17.0...@aurorasoft/fast-element@0.18.0) (2020-09-10)
...
# [0.17.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.16.0...@aurorasoft/fast-element@0.17.0) (2020-08-27)
...
# [0.16.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.15.2...@aurorasoft/fast-element@0.16.0) (2020-08-13)
...
## [0.15.2](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.15.1...@aurorasoft/fast-element@0.15.2) (2020-07-31)
...
## [0.15.1](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.15.0...@aurorasoft/fast-element@0.15.1) (2020-07-23)
...
# [0.15.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.14.0...@aurorasoft/fast-element@0.15.0) (2020-07-14)
...
# [0.14.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.13.0...@aurorasoft/fast-element@0.14.0) (2020-07-02)
...
# [0.13.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.12.0...@aurorasoft/fast-element@0.13.0) (2020-06-26)
...
# [0.12.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.11.0...@aurorasoft/fast-element@0.12.0) (2020-06-17)
...
# [0.11.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.10.2...@aurorasoft/fast-element@0.11.0) (2020-06-15)
...
## [0.10.1](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.10.0...@aurorasoft/fast-element@0.10.1) (2020-06-09)
...
# [0.10.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.9.0...@aurorasoft/fast-element@0.10.0) (2020-06-05)
...
# [0.9.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.8.1...@aurorasoft/fast-element@0.9.0) (2020-05-18)
...
## [0.8.1](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.8.0...@aurorasoft/fast-element@0.8.1) (2020-04-29)
...
# [0.8.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.7.0...@aurorasoft/fast-element@0.8.0) (2020-04-27)
...
# [0.7.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.6.0...@aurorasoft/fast-element@0.7.0) (2020-04-22)
...
# [0.6.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.5.0...@aurorasoft/fast-element@0.6.0) (2020-04-10)
...
# [0.5.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.4.0...@aurorasoft/fast-element@0.5.0) (2020-04-02)
...
# [0.4.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.3.1...@aurorasoft/fast-element@0.4.0) (2020-03-30)
...
## [0.3.1](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.3.0...@aurorasoft/fast-element@0.3.1) (2020-03-25)
...
# [0.3.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.2.1...@aurorasoft/fast-element@0.3.0) (2020-03-25)
...
## [0.2.1](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.2.0...@aurorasoft/fast-element@0.2.1) (2020-03-18)
...
# [0.2.0](https://github.com/Microsoft/fast/compare/@aurorasoft/fast-element@0.1.0...@aurorasoft/fast-element@0.2.0) (2020-03-13)
