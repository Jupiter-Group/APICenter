...
```shell
npm install --save @aurorasoft/fast-element
```
...
<html lang="en">
    <head>
        <script type="module">
          import { FASTElement } from "https://cdn.jsdelivr.net/npm/@aurorasoft/fast-element/dist/fast-element.min.js";
...
```html
<script type="module" src="https://cdn.jsdelivr.net/npm/@aurorasoft/fast-element@1.6.2/dist/fast-element.min.js"></script>
```
...
importing `@aurorasoft/fast-element`.
...
`@aurorasoft/fast-element/debug.js` exports `enableDebug()` instead of configuring
FAST at import time. The development root export and debug rollup bundle still
enable debug behavior automatically.

```ts
import { enableDebug } from "@aurorasoft/fast-element/debug.js";
...
The root `@aurorasoft/fast-element` entrypoint exports the FAST Element
...
`@aurorasoft/fast-element/registry.js` for focused definition lookups and
...
`@aurorasoft/fast-element`:

```ts
import { ElementStyles, css } from "@aurorasoft/fast-element";
```
...
`@aurorasoft/fast-element/declarative.js`. This entrypoint exports the
...
`enableHydration()` from `@aurorasoft/fast-element/hydration.js`.
...
```ts
import { attributeMap } from "@aurorasoft/fast-element/attribute-map.js";
import { declarativeTemplate } from "@aurorasoft/fast-element/declarative.js";
import { observerMap } from "@aurorasoft/fast-element/observer-map.js";
...
from `@aurorasoft/fast-element` and pass it on the element definition;
...
`@aurorasoft/fast-element/declarative-utilities.js`. See
...
Hydration of prerendered content is **opt-in**. Call `enableHydration()` from `@aurorasoft/fast-element/hydration.js` before any FAST elements connect to activate the hydration path:
...
```typescript
import { FASTElement, type FASTElementExtension } from "@aurorasoft/fast-element";
