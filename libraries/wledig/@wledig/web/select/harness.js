...
export class SelectHarness extends Harness {
    ...
    async clickOption(index) {
        const menu = this.element.renderRoot.querySelector('wd-menu');
        if (!menu.open) {
    ...
    get isOpen() {
        const menu = this.element.renderRoot.querySelector('wd-menu');
        if (!menu) {
            throw new Error('Internal wd-menu is not found. wd-select may not have finished rendering when isOpen has been checked');
        }
        return menu.open;
    }
}
