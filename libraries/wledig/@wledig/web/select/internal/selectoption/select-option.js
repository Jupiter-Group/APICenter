 ...
 * @fires request-selection {Event} Requests the parent wd-select to select this
 * element (and deselect others if single-selection) when `selected` changed to
 * `true`. --bubbles --composed
 * @fires request-deselection {Event} Requests the parent wd-select to deselect
...
export class SelectOptionEl extends selectOptionBaseClass {
     ...
     */
    renderRipple() {
        return html ` <wd-ripple
      part="ripple"
      for="item"
      ?disabled=${this.disabled}></wd-ripple>`;
    }
    /**
     * Handles rendering of the focus ring.
     */
    renderFocusRing() {
        return html ` <wd-focus-ring
      part="focus-ring"
      for="item"
      inward></wd-focus-ring>`;
    }
    /**
...
__decorate([
    property({ type: Boolean, attribute: 'wd-menu-item', reflect: true })
