...
import { a as ContextRequestEvent, n as mediaContext, r as playerContext, t as containerContext } from "./context-0rI_P6jf.js";
import { t as anyAbortSignal } from "./abort-DsTff8mS.js";
import { t as shallowEqual } from "./shallow-equal-Daj3-_LC.js";
...
const INTERACTIVE_SELECTOR = "button,input,select,textarea,a[href],[role=\"slider\"],[role=\"button\"],[data-interactive]";
...
const STORE_SYMBOL = Symbol("@webplayer/store");
...
//#endregion
//#region ../core/dist/dev/dom/presentation/cast.js
...
function isCastConnected(media) {
	return resolveRemote(media)?.state === "connected";
}
function isCastConnecting(media) {
	return resolveRemote(media)?.state === "connecting";
}
async function requestCast(media) {
...
//#endregion
//#region ../core/dist/dev/dom/store/features/cast.js
const castFeature = definePlayerFeature({
	name: "cast",
	state: ({ target }) => ({
		...
		async toggleCast() {
			...
			if (isCastConnected(media)) return requestCast(media);
	...
	attach({ target, signal, set }) {
		...
		const syncState = () => set({ castState: media.remote.state });
		syncState();
...
function isPictureInPictureElement(media) {
	...
	if (target instanceof HTMLVideoElement) return target.webkitPresentationMode === "picture-in-picture";
	return false;
}
async function requestPictureInPicture(media) {
	const target = resolveMediaTarget(media);
	if (!(target instanceof HTMLVideoElement)) throw new DOMException("Picture-in-Picture not supported", "NotSupportedError");
	const video = target;
...
const signalKeys = { seek: Symbol.for("@webplayer/seek") };
...
//#endregion
export { ContextProvider as A, AbortControllerRegistry as C, isUndefined as D, isNumber as E, ReactiveElement as M, isEditableTarget as N, MediaContainerElement as O, isInteractiveActivation as P, throwNoTargetError as S, isNull as T, getGestureCoordinator as _, backgroundFeatures as a, createState as b, timeFeature as c, playbackRateFeature as d, playbackFeature as f, controlsFeature as g, errorFeature as h, audioFeatures as i, ContextConsumer as j, MediaElement as k, textTrackFeature as l, fullscreenFeature as m, PlayerController as n, videoFeatures as o, pipFeature as p, SnapshotController as r, volumeFeature as s, createPlayer as t, sourceFeature as u, castFeature as v, isFunction as w, noop as x, bufferFeature as y };
//# sourceMappingURL=create-player-BKLw7l1a.js.map